-- Fix Rate Limits in Supabase - SIMPLIFIED VERSION
...
-- 5. Create function to check if signup is allowed (SIMPLIFIED)
...
-- 7. Create simple function to get signup status
CREATE OR REPLACE FUNCTION get_signup_status_simple(user_email TEXT)
RETURNS TABLE(
    email_address TEXT,
    total_attempts INTEGER,
...
GRANT EXECUTE ON FUNCTION get_signup_status_simple(TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION get_signup_status_simple(TEXT) TO anon;
...
-- Test the simple get_signup_status function
SELECT * FROM get_signup_status_simple('test@example.com');
