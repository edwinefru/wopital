-- Safe fix for RLS recursion issues - only operates on existing tables
-- This script will fix the infinite recursion error without causing table errors
...
BEGIN
    ...
    LOOP
        IF EXISTS (
            ...
            EXECUTE format('GRANT ALL ON %I TO anon, authenticated', table_name);
            RAISE NOTICE 'Granted permissions on table: %', table_name;
        ELSE
            RAISE NOTICE 'Table does not exist, skipping permissions: %', table_name;
        END IF;
    END LOOP;
END $$;
